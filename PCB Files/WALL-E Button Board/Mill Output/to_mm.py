    ...
    with open(output_file, 'w') as output_file:
        ...
        for line in lines:
            ...
            if line.startswith(('G0', 'G1', 'G2', 'G3')):
                ...
                k_match = re.search(r'K(-?\d+\.?\d*)', line)
                ...
                if i_match:
                    i_value = float(i_match.group(1)) * conversion_factor
                    line = re.sub(r'I(-?\d+\.?\d*)', f'I{i_value:.4f}', line)
                if j_match:
                    j_value = float(j_match.group(1)) * conversion_factor
                    line = re.sub(r'J(-?\d+\.?\d*)', f'J{j_value:.4f}', line)
                if k_match:
                    k_value = float(k_match.group(1)) * conversion_factor
                    line = re.sub(r'K(-?\d+\.?\d*)', f'K{k_value:.4f}', line)
                if f_match:
